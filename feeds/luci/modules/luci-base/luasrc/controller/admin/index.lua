...
function index()
	...
	page = node("admin", "custom")
	page.title = _("Custom")
	page.order = 5
	page.index = true
	toplevel_page(page, "admin/custom/overview", alias("admin", "custom", "overview"))

	local fs = require "nixio.fs"                                                     
        local acc = fs.access("/tmp/luci-system-dir")
	...
	entry({"admin", "change"}, call("action_change"), _("SYSChange"), 998)
	-- Logout is last
...
function action_change()
	...
	local acc = fs.access("/tmp/luci-system-dir")
	if acc then
		fs.remove("/tmp/luci-system-dir")
	else
		fs.mkdir("/tmp/luci-system-dir")
	end
	luci.http.redirect("custom/overview")
end
