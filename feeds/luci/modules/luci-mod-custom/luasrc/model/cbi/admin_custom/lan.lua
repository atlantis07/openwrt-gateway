...
m.redirect = luci.dispatcher.build_url("admin", "custom", "lan")
m:chain("wireless")
m:chain("luci")
...
if not net then
	luci.http.redirect(luci.dispatcher.build_url("admin/custom/lan"))
	return
end
...
s = m:section(NamedSection, "lan", "interface", translate("Common Configuration"))
s.addremove = false
...
if has_dnsmasq and net:proto() == "static" then
	...
	elseif has_section then
		...
		function s.filter(self, section)
			return m2.uci:get("dhcp", section, "interface") == "lan"
		end
